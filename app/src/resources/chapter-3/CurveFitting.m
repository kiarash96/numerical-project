...
if(type == 1)
    ...
    a = exp(a1);
    b = b1 ;
    result = strcat('y = ' , sprintf('%0.5f',double(b)) , ' + e ^' ,  sprintf('%0.5f',double(a)) , 'x');
     figure
      plot(X1 , Y);
...
elseif(type == 2)
    ...
    result = strcat('y = ' , sprintf('%0.5f',double(a)) , ' * lnx + ' ,  sprintf('%0.5f',double(b)) );
...
elseif(type == 3)
    X = vpa(1/X1);
    [a , b ] = linearCurveFitting(X , Y1 , m);
     result = strcat('y = ' , sprintf('%0.5f',double(a)) , ' / x + ' ,  sprintf('%0.5f',double(b)) );
...
elseif(type == 4)
    ...
    [a , b] = linearCurveFitting(X1 , Y , m );
     result = strcat('1 / y =  ' , sprintf('%0.5f',double(a)) , ' * x + ' ,  sprintf('%0.5f',double(b)) );
